                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.content-tags.update", [$contentTag->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            <label for="description">{{ trans('cruds.contentTag.fields.description') }}</label>
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $contentTag->description) }}">
                            @if($errors->has('description'))
                                <span class="help-block" role="alert">{{ $errors->first('description') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.contentTag.fields.description_helper') }}</span>
                        </div>
